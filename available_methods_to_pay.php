...
if (isset($_POST) && !empty($_POST)){
    ...
    $response = $initiatePayment->setCurlFieldsThenExecuteCurl(
            [
                'InvoiceAmount' => $_POST['price'],
                'CurrencyIso'   => $_POST['currency']
            ]);

    if ($initiatePayment->curlErrorExist()) {
            ...
            <div class="row">
                ...
                <?php
                    if (isset($json['Data']['PaymentMethods']) && !empty($json['Data']['PaymentMethods'])){
                        ...
                        echo 'Sorry!, There is no method available now, for more details you can print [json] variable';
                    }
                ?>
            </div>
        </div>
